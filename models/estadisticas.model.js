...
WHERE p.fecha >= CONVERT_TZ(DATE_SUB(NOW(), INTERVAL 7 DAY), 'UTC', 'America/La_Paz')
...
WHERE pd.fecha >= CONVERT_TZ(DATE_SUB(NOW(), INTERVAL 7 DAY), 'UTC', 'America/La_Paz')
  AND c.anulado = FALSE
...
WHERE p.fecha BETWEEN 
  CONVERT_TZ(DATE_SUB(NOW(), INTERVAL 14 DAY), 'UTC', 'America/La_Paz') AND 
  CONVERT_TZ(DATE_SUB(NOW(), INTERVAL 7 DAY), 'UTC', 'America/La_Paz')
    `);
